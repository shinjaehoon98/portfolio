...
let activeImg;//변수선언
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    hoverTexts.forEach((textElement, index) => {
      textElement.addEventListener('mousemove', function (e) {
        var hoverImage = hoverImages[index];
        hoverImage.style.display = 'block';
        var imgWidth = hoverImage.clientWidth;
        var imgHeight = hoverImage.clientHeight;
        hoverImage.style.left = e.pageX + 'px'; // 이미지의 가운데가 마우스 커서에 위치하도록 조정
        hoverImage.style.top = e.pageY + 'px'; // 이미지의 가운데가 마우스 커서에 위치하도록 조정
      });
